...
namespace processing.core
{
	...
	using processing.@event;
	//using processing.opengl;
	...
	/// </summary>
	public class PApplet : UnityEngine.MonoBehaviour, PConstants
	{
		...
		{
			eventQueue = new InternalEventQueue(this);
		}
		...
		/// </para>
		/// </summary>
		public static readonly float javaVersion = (float)Convert.ToSingle(javaVersionName.Substring(0, 3));
		...
		static PApplet()
		{
			...
			{
				platform = PConstants_Fields.MACOSX;

			}
			else if (osname.IndexOf("Windows") != -1)
			{
				platform = PConstants_Fields.WINDOWS;
			} // true for the ibm vm
			else if (osname.Equals("Linux"))
			{
				platform = PConstants_Fields.LINUX;

			}
			else
			{
				platform = PConstants_Fields.OTHER;
			}
		}
		...
		public static bool useQuartz = false;
		...
		/// </summary>
		public static bool useNativeSelect = (platform != PConstants_Fields.LINUX);
		...
		/// <summary>
		/// The PGraphics renderer associated with this PApplet </summary>
		public PGraphics g;

		/// <summary>
		/// The frame containing this applet (if any) </summary>
		// TODO: comment out by hirokiprocessing.coreのコード見たらファイル数とか対したことなくてコレイケるんじゃね？とか思ったけど、やっぱり結構ハードル高くて無心で
//		public Frame frame;

		// disabled on retina inside init()
		internal bool useActive = true;
		//  boolean useActive = false;
		//  boolean useStrategy = true;
		internal bool useStrategy = false;

		// TODO: commented out by hirokiprocessing.coreのコード見たらファイル数とか対したことなくてコレイケるんじゃね？とか思ったけど、やっぱり結構ハードル高くて無心で
//		internal Canvas canvas;
		...
		/// <summary>
		/// Path to sketch folder </summary>
		public string sketchPath_Renamed; //folder;

		internal const bool DEBUG = false;
		//  static final boolean DEBUG = true;

		/// <summary>
		/// Default width and height for applet when not specified </summary>
		...
		/// </summary>
		public const int MIN_WINDOW_WIDTH = 128;
		public const int MIN_WINDOW_HEIGHT = 128;
		...
		/// </para>
		/// </summary>
		public class RendererChangeException : Exception
		{
		}
		...
		/// <summary>
		/// Storage for the current renderer size to avoid re-allocation. </summary>
		internal UnityEngine.Rect currentSize = new UnityEngine.Rect();
		...
		/// ( end auto-generated )
		///
		/// @webref image:pixels </summary>
		/// <seealso cref= PApplet#loadPixels() </seealso>
		/// <seealso cref= PApplet#updatePixels() </seealso>
		/// <seealso cref= PApplet#get(int, int, int, int) </seealso>
		...
		/// <seealso cref= PApplet#mouseDragged()
		///
		///  </seealso>
		public int mouseX;
		...
		/// ( end auto-generated )
		/// @webref input:mouse </summary>
		/// <seealso cref= PApplet#pmouseY </seealso>
		/// <seealso cref= PApplet#mouseX </seealso>
		/// <seealso cref= PApplet#mouseY </seealso>
		public int pmouseX;
		...
		/// ( end auto-generated )
		/// @webref input:mouse </summary>
		/// <seealso cref= PApplet#pmouseX </seealso>
		/// <seealso cref= PApplet#mouseX </seealso>
		/// <seealso cref= PApplet#mouseY </seealso>
		public int pmouseY;
		...
		/// </summary>
		protected internal int dmouseX, dmouseY;
		...
		/// </summary>
		protected internal int emouseX, emouseY;
		...
		[Obsolete("Please refrain from using this variable, it will be removed")]
		...
		/// <seealso cref= PApplet#mouseMoved() </seealso>
		/// <seealso cref= PApplet#mouseDragged() </seealso>
		public bool mousePressed_Renamed;


		/// @deprecated Use a mouse event handler that passes an event instead.
		[Obsolete("Use a mouse event handler that passes an event instead.")]
		...
		/// <seealso cref= PApplet#keyPressed() </seealso>
		/// <seealso cref= PApplet#keyReleased() </seealso>
		public bool keyPressed_Renamed;


		/// <summary>
		/// The last KeyEvent object passed into a mouse function. </summary>
		/// @deprecated Use a key event handler that passes an event instead.
		[Obsolete("Use a key event handler that passes an event instead.")]
		...
		/// </para>
		/// </summary>
		internal long millisOffset = DateTimeHelperClass.CurrentUnixTimeMillis();
		...
		///
		/// ( end auto-generated )
		/// @webref environment </summary>
		/// <seealso cref= PApplet#frameRate(float) </seealso>
		public float frameRate_Renamed = 10;
		/// <summary>
		/// Last time in nanoseconds that frameRate was checked </summary>
		protected internal long frameRateLastNanos = 0;

		/// <summary>
		/// As of release 0116, frameRate(60) is called as a default </summary>
		protected internal float frameRateTarget = 60;
		protected internal long frameRatePeriod = 1000000000L / 60L;

		protected internal bool looping;

		/// <summary>
		/// flag set to true when a redraw is asked for by the user </summary>
		protected internal bool redraw_Renamed;
		...
		/// <summary>
		/// true if the sketch has stopped permanently. </summary>
		...
		/// </summary>
		protected internal bool exitCalled;

		internal object pauseObject = new object();
		internal Thread thread_Renamed;
		...
		/// </summary>
		public const string ARGS_EDITOR_LOCATION = "--editor-location";
		...
		/// </para>
		/// </summary>
		public const string ARGS_EXTERNAL = "--external";

		public const string ARGS_LOCATION = "--location";

		public const string ARGS_DISPLAY = "--display";

		public const string ARGS_BGCOLOR = "--bgcolor";

		/// @deprecated use --full-screen instead.
		public const string ARGS_PRESENT = "--present";

		public const string ARGS_FULL_SCREEN = "--full-screen";

		//  static public final String ARGS_EXCLUSIVE = "--exclusive";

		public const string ARGS_STOP_COLOR = "--stop-color";

		public const string ARGS_HIDE_STOP = "--hide-stop";
		...
		/// </para>
		/// </summary>
		public const string ARGS_SKETCH_FOLDER = "--sketch-path";
		...
		public const string EXTERNAL_STOP = "__STOP__";
		...
		/// </para>
		/// </summary>
		public const string EXTERNAL_MOVE = "__MOVE__";

		/// <summary>
		/// true if this sketch is being run by the PDE </summary>
		internal bool external = false;

		/// <summary>
		/// Not official API, using internally because of the tweaks required.
		/// </summary>
		internal bool retina;


		internal const string ERROR_MIN_MAX = "Cannot use min() or max() on an empty array.";
...
//				online = false;
			...
			// TODO: porting to Unity
			// parhaps doen't need this
			//addComponentListener(new ComponentAdapterRedrawHelper(this));

			//    if (thread == null) {
			//    paused = true;
			thread_Renamed = new Thread(this, "Animation Thread");
		...
		// TODO: porting to Unity
		// perhaps doesn't need this
//		private class ComponentAdapterRedrawHelper : ComponentAdapter
//		{
//			private readonly PApplet outerInstance;
//
//			public ComponentAdapterRedrawHelper(PApplet outerInstance)
//			{
...
//		}
		...
		private bool checkRetina()
		{
			if (platform == PConstants_Fields.MACOSX)
			{
				...
				//ORIGINAL LINE: final String javaVendor = System.getProperty("java.vendor");
				string javaVendor = System.getProperty("java.vendor");
				if (javaVendor.Contains("Apple"))
				{
					float? prop = (float? ) Toolkit.getDesktopProperty("apple.awt.contentScaleFactor");
					if (prop != null)
					{
						return prop == 2;
					}
				...
				else if (javaVendor.Contains("Oracle"))
				{
					...
					if (m != null && PApplet.parseInt(m[1]) >= 7 && PApplet.parseInt(m[1]) >= 40)
					{
						...
						GraphicsDevice device = env.DefaultScreenDevice;

						try
						{
							Field field = device.GetType().getDeclaredField("scale");
							if (field != null)
							{
								field.Accessible = true;
								object scale = field.get(device);

								if (scale is int? && (int)((int? )scale) == 2)
								{
									return true;
								}
							}
		...
		public virtual int sketchQuality()
		{
			return 2;
		}


		public virtual int sketchWidth()
		{
			return DEFAULT_WIDTH;
		}


		public virtual int sketchHeight()
		{
			return DEFAULT_HEIGHT;
		}


		public virtual string sketchRenderer()
		{
			return PConstants_Fields.JAVA2D;
		}


		public virtual bool sketchFullScreen()
		{
			//    return fullScreen;
			return false;
		}
		...
		public virtual void orientation(int which)
		{
		...
		public void start()
		{
			...
			lock (pauseObject)
			{
				...
				//      try {
				Monitor.PulseAll(pauseObject); // wake up the animation thread
		...
		/// <summary>
		/// Map of registered methods, stored by name. </summary>
		internal Dictionary<string, RegisteredMethods> registerMap = new Dictionary<string, PApplet.RegisteredMethods>();


		internal class RegisteredMethods
		{
			private readonly PApplet outerInstance;

			public RegisteredMethods(PApplet outerInstance)
			{
				this.outerInstance = outerInstance;
			}
			...
			internal virtual void handle(object[] args)
			{
			...
			internal virtual void add(object @object, System.Reflection.MethodInfo method)
			{
				...
				else
				{
					...
					outerInstance.die(method.Name + "() already added for this instance of " + @object.GetType().FullName);
				}
			}
			...
			protected internal virtual int findIndex(object @object)
			{
				for (int i = 0; i < count; i++)
				{
					if (objects[i] == @object)
		...
		/// <ul>
		/// <li>pre 窶� at the very top of the draw() method (safe to draw)
		/// <li>draw 窶� at the end of the draw() method (safe to draw)
		/// <li>post 窶� after draw() has exited (not safe to draw)
		/// <li>pause 窶督�called when the sketch is paused
		/// <li>resume 窶督�called when the sketch is resumed
		/// <li>dispose 窶� when the sketch is shutting down (definitely not safe to draw)
		/// <ul>
		...
		/// See the Library Wiki page for more details. </summary>
		/// <param name="methodName"> name of the method to be called </param>
		/// <param name="target"> the target object that should receive the event </param>
		public virtual void registerMethod(string methodName, object target)
		{
			if (methodName.Equals("mouseEvent"))
			{
				registerWithArgs("mouseEvent", target, new Type[] {typeof(processing.@event.MouseEvent)});

			}
			else if (methodName.Equals("keyEvent"))
			{
				registerWithArgs("keyEvent", target, new Type[] {typeof(processing.@event.KeyEvent)});

			}
			else if (methodName.Equals("touchEvent"))
			{
				registerWithArgs("touchEvent", target, new Type[] {typeof(processing.@event.TouchEvent)});
		...
		{
			RegisteredMethods meth = registerMap[name];
			if (meth == null)
			{
			...
			try
			{
				Method method = c.GetMethod(name, new Type[] {});
				meth.add(o, method);
			...
			{
				//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
				die("There is no public " + name + "() method in the class " + o.GetType().FullName);
		...
		private void registerWithArgs(string name, object o, Type[] cargs)
		{
			RegisteredMethods meth = registerMap[name];
			if (meth == null)
			{
		...
		public virtual void unregisterMethod(string name, object target)
		{
			RegisteredMethods meth = registerMap[name];
			if (meth == null)
			{
		...
		protected internal virtual void handleMethods(string methodName)
		{
			RegisteredMethods meth = registerMap[methodName];
			if (meth != null)
			{
				meth.handle();
			}
		...
		protected internal virtual void handleMethods(string methodName, object[] args)
		{
			RegisteredMethods meth = registerMap[methodName];
			if (meth != null)
			{
		...
		[Obsolete]
		public virtual void registerSize(object o)
		{
			Console.Error.WriteLine("The registerSize() command is no longer supported.");
		...
		[Obsolete]
		public virtual void registerPre(object o)
		{
		...
		[Obsolete]
		public virtual void registerDraw(object o)
		{
		...
		[Obsolete]
		public virtual void registerPost(object o)
		{
		...
		[Obsolete]
		public virtual void registerDispose(object o)
		{
		...
		[Obsolete]
		public virtual void unregisterSize(object o)
		{
			Console.Error.WriteLine("The unregisterSize() command is no longer supported.");
			//    Class<?> methodArgs[] = new Class[] { Integer.TYPE, Integer.TYPE };
		...
		[Obsolete]
		public virtual void unregisterPre(object o)
		{
		...
		[Obsolete]
		public virtual void unregisterDraw(object o)
		{
		...
		[Obsolete]
		public virtual void unregisterPost(object o)
		{
		...
		[Obsolete]
		public virtual void unregisterDispose(object o)
		{
		...
		internal RegisteredMethods mouseEventMethods, keyEventMethods;


		protected internal virtual void reportDeprecation(Type c, bool mouse)
		{
			if (g != null)
			{
				//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
				PGraphics.showWarning("The class " + c.FullName + " is incompatible with Processing 2.0.");
				...
				if (g is PGraphicsOpenGL)
				{
					...
					throw new Exception("This sketch uses a library that " + "needs to be updated for Processing 2.0.");
				}
			}
		}
		...
		[Obsolete]
		public virtual void registerMouseEvent(object o)
		{
			Type c = o.GetType();
			reportDeprecation(c, true);
			try
			{
				Method method = c.GetMethod("mouseEvent", new Type[] {typeof(java.awt.@event.MouseEvent)});
				if (mouseEventMethods == null)
				{
					mouseEventMethods = new RegisteredMethods(this);
				}
		...
		[Obsolete]
		public virtual void unregisterMouseEvent(object o)
		{
			try
			{
		...
		[Obsolete]
		public virtual void registerKeyEvent(object o)
		{
			Type c = o.GetType();
			reportDeprecation(c, false);
			try
			{
				Method method = c.GetMethod("keyEvent", new Type[] {typeof(java.awt.@event.KeyEvent)});
				if (keyEventMethods == null)
				{
					keyEventMethods = new RegisteredMethods(this);
				}
		...
		[Obsolete]
		public virtual void unregisterKeyEvent(object o)
		{
			try
			{
		...
		/// <seealso cref= PApplet#loop() </seealso>
		/// <seealso cref= PApplet#noLoop() </seealso>
		/// <seealso cref= PApplet#draw() </seealso>
		...
		/// @webref structure
		/// @usage web_application </summary>
		/// <seealso cref= PApplet#setup() </seealso>
		/// <seealso cref= PApplet#loop() </seealso>
		/// <seealso cref= PApplet#noLoop() </seealso>
		/// <seealso cref= PApplet#redraw() </seealso>
		/// <seealso cref= PApplet#frameRate(float) </seealso>
		...
		protected internal virtual void resizeRenderer(int newWidth, int newHeight)
		{
		...
		/// </para>
		/// </summary>
		/// <param name="w"> width of the display window in units of pixels </param>
		/// <param name="h"> height of the display window in units of pixels </param>
		public virtual void size(int w, int h)
		{
			size(w, h, PConstants_Fields.JAVA2D, null);
		}

		/// <param name="renderer"> Either P2D, P3D, or PDF </param>
		public virtual void size(int w, int h, string renderer)
		{
		...
		//ORIGINAL LINE: public void size(final int w, final int h, String renderer, String path)
		public virtual void size(int w, int h, string renderer, string path)
		{
			// Run this from the EDT, just cuz it's AWT stuff (or maybe later Swing)
				EventQueue.invokeLater(new DimentionSizeClassHelper(this, w, h));
			...
			//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
			string currentRenderer = g.GetType().FullName;
		...
		private class DimentionSizeClassHelper : Runnable
		{
			private readonly PApplet outerInstance;
			...
			private int h;
				...
				public DimentionSizeClassHelper(PApplet outerInstance, int w, int h)
			{
				this.outerInstance = outerInstance;
				this.w = w;
				this.h = h;
		...
		public virtual PGraphics createGraphics(int w, int h)
		{
			return createGraphics(w, h, PConstants_Fields.JAVA2D);
		}
		...
		/// <param name="renderer"> Either P2D, P3D, or PDF
		/// </param>
		/// <seealso cref= PGraphics#PGraphics
		///  </seealso>
		public virtual PGraphics createGraphics(int w, int h, string renderer)
		{
		...
		/// for a renderer that writes to a file (such as PDF or DXF). </summary>
		/// <param name="path"> the name of the file (can be an absolute or relative path) </param>
		public virtual PGraphics createGraphics(int w, int h, string renderer, string path)
		{
			if (path != null)
			{
				path = savePath(path);
		...
		/// </summary>
		protected internal virtual PGraphics makeGraphics(int w, int h, string renderer, string path, bool primary)
		{
			string openglError = external ? "Before using OpenGL, first select " + "Import Library > OpenGL from the Sketch menu." : "The Java classpath and native library path is not " + "properly set for using the OpenGL library."; // welcome to Java programming!

			if (!primary && !g.GL)
			{
				if (renderer.Equals(PConstants_Fields.P2D))
				{
					throw new Exception("createGraphics() with P2D requires size() to use P2D or P3D");
				}
				else if (renderer.Equals(PConstants_Fields.P3D))
				{
					throw new Exception("createGraphics() with P3D or OPENGL requires size() to use P2D or P3D");
				}
			}
			...
			try
			{
				Type rendererClass = Thread.CurrentThread.ContextClassLoader.loadClass(renderer);

				//JAVA TO C# CONVERTER TODO TASK: Java wildcard generics are not converted to .NET:
				//ORIGINAL LINE: Constructor<?> constructor = rendererClass.getConstructor(new Class[] { });

				//TODO: implement this

				//              Constructor<?> constructor = rendererClass.GetConstructor(new Type[] { });
			...
			{
				string msg = ite.TargetException.Message;
				if ((msg != null) && (msg.IndexOf("no jogl in java.library.path") != -1))
				{
					throw new Exception(openglError + " (The native library is missing.)");
				}
				else
				{
					ite.TargetException.printStackTrace();
					Exception target = ite.TargetException;
					if (platform == PConstants_Fields.MACOSX) // bug
					{
						// instead of printing StackTrace, LogException
						UnityEngine.Debug.LogException(target);
					}
			...
			{
				if (cnfe.Message.IndexOf("processing.opengl.PGraphicsOpenGL") != -1)
				{
					throw new Exception(openglError + " (The library .jar file is missing.)");
				}
				else
				{
					throw new Exception("You need to use \"Import Library\" " + "to add " + renderer + " to your sketch.");
				}

			}
			catch (Exception e)
			{
				if ((e is System.ArgumentException) || (e is NoSuchMethodException) || (e is IllegalAccessException))
				{
					if (e.Message.contains("cannot be <= 0"))
					{
		...
		*/
		...
		public void paint(Graphics screen)
		{
			...
			*/
			...
			//      new Exception().printStackTrace(System.out);
			//    }
			if (!insideDraw && (g != null) && (g.image_Renamed != null))
			{
				if (useStrategy)
				{
					render();
				...
				else
				{
					...
					screen.drawImage(g.image_Renamed, 0, 0, width, height, null);
				}
			}
			else
			{
				debug(insideDraw + " " + g + " " + ((g != null) ? g.image_Renamed : "-"));
			}
		}
		...
		protected internal virtual void render()
		{
			lock (this)
			{
				...
				if (canvas.BufferStrategy == null) // whole block [121222]
				{
					//      System.out.println("creating a strategy");
					canvas.createBufferStrategy(2);
				}
				BufferStrategy strategy = canvas.BufferStrategy;
				if (strategy == null)
				{
					return;
				}
				...
				do
				{
					...
					do
					{
						Graphics draw = strategy.DrawGraphics;
						draw.drawImage(g.image_Renamed, 0, 0, width, height, null);
						draw.dispose();
		...
		*/
		...
		/// </summary>
		public virtual void run() // not good to make this synchronized, locks things up
		{
			...
			const int NO_DELAYS_PER_YIELD = 15;
			...
			*/
			...
			while ((Thread.CurrentThread == thread_Renamed) && !finished)
			{
				if (paused)
				{
				...
				if (frameCount == 1)
				{
					...
					EventQueue.invokeLater(new FocusRequestsHelper(this));
				}
		...
		private class FocusRequestsHelper : Runnable
		{
			private readonly PApplet outerInstance;

				public FocusRequestsHelper(PApplet outerInstance)
			{
				this.outerInstance = outerInstance;
			}
			...
			public virtual void run()
			{
				...
				outerInstance.g.requestFocus();
			}
		}


		protected internal bool insideDraw;

		//synchronized public void handleDisplay() {
		public virtual void handleDraw()
		{
			debug("handleDraw() " + g + " " + looping + " " + redraw_Renamed + " valid:" + this.Valid + " visible:" + this.Visible);
			if (canDraw())
			{
				if (!g.canDraw())
				{
				...
				if (frameCount == 0)
				{
					...
					GraphicsDevice displayDevice = GraphicsConfiguration.Device;
					if (displayDevice == null)
					{
						return;
					}
					Rectangle screenRect = displayDevice.DefaultConfiguration.Bounds;
				...
				else
				{
					...
					frameRate_Renamed = (frameRate_Renamed * 0.9f) + (instantaneousRate * 0.1f);
					...
					redraw_Renamed = false; // unset 'redraw' flag in case it was set
		...
		/// <summary>
		/// Not official API, not guaranteed to work in the future. </summary>
		public virtual bool canDraw()
		{
			return g != null && (looping || redraw_Renamed);
		}
		...
		public virtual void addListeners(Component comp)
		{
		...
		public virtual void removeListeners(Component comp)
		{
		...
		/// </summary>
		public virtual void updateListeners(Component comp)
		{
		...
		internal class InternalEventQueue
		{
			...
			public InternalEventQueue(PApplet outerInstance)
			{
				this.outerInstance = outerInstance;
			}

			protected internal Event[] queue = new Event[10];
			protected internal int offset;
			protected internal int count;

			internal virtual void add(Event e)
			{
				lock (this)
				{
			...
			internal virtual Event remove()
			{
				lock (this)
				{
					if (offset == count)
					{
						throw new Exception("Nothing left on the event queue.");
					}
			...
			internal virtual bool available()
			{
				lock (this)
				{
					return count != 0;
		...
		internal InternalEventQueue eventQueue;
		...
		/// </summary>
		public virtual void postEvent(Event pe)
		{
		...
		protected internal virtual void dequeueEvents()
		{
		...
		/// </summary>
		protected internal virtual void handleMouseEvent(MouseEvent @event)
		{
			...
			if (@event.Action == MouseEvent.DRAG || @event.Action == MouseEvent.MOVE)
			{
				pmouseX = emouseX;
				pmouseY = emouseY;
				mouseX = @event.X;
			...
			if (mouseEventMethods != null)
			{
				...
				{
					mouseEventMethods.handle(new object[] {@event.Native});
				}
			}
			...
			case MouseEvent.PRESS:
				mousePressed_Renamed = true;
				break;
			case MouseEvent.RELEASE:
				mousePressed_Renamed = false;
				break;
			}
...
//
		...
		/// </para>
		/// </summary>
		/// <seealso cref= PApplet#second() </seealso>
		/// <seealso cref= PApplet#minute() </seealso>
		/// <seealso cref= PApplet#hour() </seealso>
		/// <seealso cref= PApplet#day() </seealso>
		/// <seealso cref= PApplet#month() </seealso>
		/// <seealso cref= PApplet#year()
		///  </seealso>
...
//			return (int)(DateTimeHelperClass.CurrentUnixTimeMillis() - millisOffset);
		...
		///
		/// ( end auto-generated )
		/// @webref input:time_date </summary>
		/// <seealso cref= PApplet#millis() </seealso>
		/// <seealso cref= PApplet#minute() </seealso>
		/// <seealso cref= PApplet#hour() </seealso>
		/// <seealso cref= PApplet#day() </seealso>
		/// <seealso cref= PApplet#month() </seealso>
		...
		/// the serial port).
		/// </summary>
		public virtual void delay(int napTime)
		{
			//if (frameCount != 0) {
			//if (napTime > 0) {
			try
		...
		/// <seealso cref= PApplet#noLoop() </seealso>
		/// <seealso cref= PApplet#redraw() </seealso>
		public virtual void frameRate(float fps)
		{
			frameRateTarget = fps;
		...
		/// </summary>
		/// <param name="name"> name of the param to read </param>
		/// @deprecated no more applet support
		public virtual string param(string name)
		{
			if (online)
			{
			...
			{
				Console.Error.WriteLine("param() only works inside a web browser");
			}
			return null;
		}
		...
		/// </summary>
		/// @deprecated no more applet support
		public virtual void status(string value)
		{
			if (online)
			{
				showStatus(value);
			...
			{
				Console.WriteLine(value); // something more interesting?
			}
		}


		public virtual void link(string url)
		{
			//    link(url, null);
			try
			{
		...
		/// </para>
		/// </summary>
		/// <param name="url"> the complete URL, as a String in quotes </param>
		/// <param name="target"> the name of the window in which to load the URL, as a String in quotes </param>
		/// @deprecated the 'target' parameter is no longer relevant with the removal of applets
		public virtual void link(string url, string target)
		{
			link(url);
			/*
			try {
...
//		}
		...
		/// </summary>
		public virtual void die(string what)
		{
			dispose();
		...
		/// </summary>
		public virtual void die(string what, Exception e)
		{
			if (e != null)
			{
				Console.WriteLine(e.ToString());
		...
		internal virtual void exitActual()
		{
			try
			{
				Environment.Exit(0);
		...
		/// </summary>
		public virtual void method(string name)
		{
			try
			{
				Method method = this.GetType().GetMethod(name, new Type[] {});
			...
			{
				//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
				Console.Error.WriteLine("There is no public " + name + "() method " + "in the class " + this.GetType().FullName);
			}
			catch (Exception e)
			{
				Console.WriteLine(e.ToString());
		...
		/// </summary>
		//JAVA TO C# CONVERTER WARNING: 'final' parameters are not allowed in .NET:
		//ORIGINAL LINE: public void thread(final String name)
		public virtual void thread(string name)
		{
			// TODO: implement thread support
//			Thread later = new ThreadAnonymousInnerClassHelper(this, name);
//			later.Start();
		}
...
//			private string name;
//
//			public ThreadAnonymousInnerClassHelper(PApplet outerInstance, string name)
//			{
//				this.outerInstance = outerInstance;
//				this.name = name;
//			}
...
//		}
		...
		///
		/// ( end auto-generated )
		/// @webref output:image </summary>
		/// <param name="filename"> any sequence of letters and numbers </param>
		/// <seealso cref= PApplet#saveFrame() </seealso>
		...
		public virtual void saveFrame()
		{
			...
			{
				Console.Error.WriteLine("Can't use saveFrame() when running in a browser, " + "unless using a signed applet.");
			}
		}
		...
		/// ( end auto-generated )
		/// @webref output:image </summary>
		/// <seealso cref= PApplet#save(String) </seealso>
		/// <seealso cref= PApplet#createGraphics(int, int, String, String) </seealso>
		/// <param name="filename"> any sequence of letters or numbers that ends with either ".tif", ".tga", ".jpg", or ".png" </param>
		public virtual void saveFrame(string filename)
		{
			try
			{
		...
		/// </summary>
		public virtual string insertFrame(string what)
		{
		...
		internal int cursorType = PConstants_Fields.ARROW; // cursor type
		internal bool cursorVisible = true; // cursor visibility flag
		//  PImage invisibleCursor;
		internal Cursor invisibleCursor;


		/// <summary>
		/// Set the cursor type </summary>
		/// <param name="kind"> either ARROW, CROSS, HAND, MOVE, TEXT, or WAIT </param>
		public virtual void cursor(int kind)
		{
			Cursor = Cursor.getPredefinedCursor(kind);
			cursorVisible = true;
		...
		/// </summary>
		public virtual void cursor(PImage img)
		{
		...
		/// </para>
		/// </summary>
		/// <seealso cref= PApplet#noCursor() </seealso>
		/// <param name="img"> any variable of type PImage </param>
		/// <param name="x"> the horizontal active spot of the cursor </param>
		/// <param name="y"> the vertical active spot of the cursor </param>
		public virtual void cursor(PImage img, int x, int y)
		{
		...
		/// @webref environment </summary>
		/// <seealso cref= PApplet#cursor()
		/// @usage Application </seealso>
		public virtual void noCursor()
		{
			...
			if (invisibleCursor == null)
			{
				...
				invisibleCursor = Toolkit.createCustomCursor(cursorImg, new Point(8, 8), "blank");
			}
		...
		/// @usage IDE </summary>
		/// <param name="what"> data to print to console </param>
		/// <seealso cref= PApplet#println() </seealso>
		/// <seealso cref= PApplet#printArray(Object) </seealso>
		/// <seealso cref= PApplet#join(String[], char) </seealso>
		public static void print(sbyte what)
		{
			UnityEngine.Debug.Log (what);
		}

		public static void print(bool what)
		{
			UnityEngine.Debug.Log (what);
		}
		...
		/// <param name="variables"> list of data, separated by commas </param>
		public static void print(params object[] variables)
		{
			StringBuilder sb = new StringBuilder();
			foreach (object o in variables)
			{
				if (sb.Length != 0)
				{
					sb.Append(" ");
		...
		*/
		...
		/// @webref output:text_area
		/// @usage IDE </summary>
		/// <seealso cref= PApplet#print(byte) </seealso>
		/// <seealso cref= PApplet#printArray(Object) </seealso>
		...
		/// <param name="what"> data to print to console </param>
		public static void println(sbyte what)
		{
			UnityEngine.Debug.Log (what);
		}

		public static void println(bool what)
		{
			UnityEngine.Debug.Log (what);
		}
		...
		/// <param name="variables"> list of data, separated by commas </param>
		public static void println(params object[] variables)
		{
		...
		*/
		...
		{
			if (what != null && what.GetType().IsArray)
			{
				printArray(what);
			}
			else
		...
		///
		/// ( end auto-generated )
		/// @webref output:text_area </summary>
		/// <param name="what"> one-dimensional array
		/// @usage IDE </param>
		/// <seealso cref= PApplet#print(byte) </seealso>
		/// <seealso cref= PApplet#println() </seealso>
		public static void printArray(object what)
		{
			...
			{
				//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
				string name = what.GetType().FullName;
				if (name[0] == '[')
				{
					switch (name[1])
					{
		...
		*/
		...
		///
		/// ( end auto-generated )
		/// @webref math:calculation </summary>
		...
		/// <seealso cref= PApplet#sqrt(float) </seealso>
		...
		/// <seealso cref= PApplet#pow(float, float) </seealso>
		/// <seealso cref= PApplet#sq(float) </seealso>
		...
		/// ( end auto-generated )
		/// @webref math:calculation </summary>
		/// <param name="n"> number greater than 0.0 </param>
		...
		/// <seealso cref= PApplet#min(float, float, float) </seealso>
		...
		*/
		...
		/// <param name="c"> third number to compare </param>
		...
		/// <param name="list"> array of numbers to compare </param>
		public static int max(int[] list)
		{
			if (list.Length == 0)
			{
				throw new System.IndexOutOfRangeException(ERROR_MIN_MAX);
			}
			int max = list[0];
		...
		/// Throws an ArrayIndexOutOfBoundsException if the array is length 0. </summary>
		...
		*/
		...
		/// <seealso cref= PApplet#max(float, float, float) </seealso>
		...
		*/
		...
		///
		/// ( end auto-generated )
		/// @webref math:trigonometry </summary>
		/// <param name="angle"> an angle in radians </param>
		/// <seealso cref= PApplet#cos(float) </seealso>
		/// <seealso cref= PApplet#tan(float) </seealso>
		...
		/// @webref math:trigonometry </summary>
		/// <param name="angle"> an angle in radians </param>
		/// <seealso cref= PApplet#sin(float) </seealso>
		/// <seealso cref= PApplet#tan(float) </seealso>
		/// <seealso cref= PApplet#radians(float) </seealso>
		...
		/// <seealso cref= PApplet#sin(float) </seealso>
		/// <seealso cref= PApplet#acos(float) </seealso>
		/// <seealso cref= PApplet#atan(float) </seealso>
		...
		/// <seealso cref= PApplet#cos(float) </seealso>
		/// <seealso cref= PApplet#asin(float) </seealso>
		/// <seealso cref= PApplet#atan(float) </seealso>
		...
		/// ( end auto-generated )
		/// @webref math:trigonometry </summary>
		/// <param name="value"> -Infinity to Infinity (exclusive) </param>
		/// <seealso cref= PApplet#tan(float) </seealso>
		/// <seealso cref= PApplet#asin(float) </seealso>
		/// <seealso cref= PApplet#acos(float) </seealso>
		...
		/// ( end auto-generated )
		/// @webref math:trigonometry </summary>
		/// <param name="radians"> radian value to convert to degrees </param>
		/// <seealso cref= PApplet#radians(float) </seealso>
		public static float degrees(float radians)
		{
			return radians * PConstants_Fields.RAD_TO_DEG;
		}
		...
		/// ( end auto-generated )
		/// @webref math:trigonometry </summary>
		/// <param name="degrees"> degree value to convert to radians </param>
		/// <seealso cref= PApplet#degrees(float) </seealso>
		public static float radians(float degrees)
		{
			return degrees * PConstants_Fields.DEG_TO_RAD;
		}
		...
		/// <seealso cref= PApplet#floor(float) </seealso>
		/// <seealso cref= PApplet#round(float) </seealso>
		...
		/// <seealso cref= PApplet#ceil(float) </seealso>
		/// <seealso cref= PApplet#round(float) </seealso>
		...
		/// <seealso cref= PApplet#dist(float, float, float, float) </seealso>
		...
		/// <param name="amt"> float between 0.0 and 1.0 </param>
		...
		/// <seealso cref= PApplet#map(float, float, float, float, float) </seealso>
		/// <seealso cref= PApplet#lerp(float, float, float) </seealso>
		...
		/// <seealso cref= PApplet#norm(float, float, float) </seealso>
		/// <seealso cref= PApplet#lerp(float, float, float) </seealso>
...
//		internal Random internalRandom;
		...
		public float random(float high)
		{
			...
			if (high == 0.0f || float.IsNaN(high) || float.IsInfinity(high) )
			{
				return 0.0f;
			}

			return UnityEngine.Random.Range (0.0f, high);
		}
		...
		///
		/// ( end auto-generated )
		/// @webref math:random </summary>
		/// <seealso cref= PApplet#random(float,float) </seealso>
		/// <seealso cref= PApplet#noise(float, float, float) </seealso>
		public float randomGaussian()
		{
			//
			// NOTE: random gausian quick & dirty implementation by Box-Muller transform
		...
		/// <seealso cref= PApplet#randomSeed(long) </seealso>
		/// <seealso cref= PApplet#noise(float, float, float) </seealso>
		...
		/// <seealso cref= PApplet#random(float,float) </seealso>
		/// <seealso cref= PApplet#noise(float, float, float) </seealso>
		/// <seealso cref= PApplet#noiseSeed(long) </seealso>
		...
		internal const int PERLIN_YWRAPB = 4;
		internal static readonly int PERLIN_YWRAP = 1 << PERLIN_YWRAPB;
		internal const int PERLIN_ZWRAPB = 8;
		internal static readonly int PERLIN_ZWRAP = 1 << PERLIN_ZWRAPB;
		internal const int PERLIN_SIZE = 4095;

		internal int perlin_octaves = 4; // default to medium smooth
		internal float perlin_amp_falloff = 0.5f; // 50% reduction/octave

		// [toxi 031112]
		// new vars needed due to recent change of cos table in PGraphics
		internal int perlin_TWOPI, perlin_PI;
		internal float[] perlin_cosTable;
		internal float[] perlin;

//		internal Random perlinRandom;
		...
		/// <seealso cref= PApplet#noiseSeed(long) </seealso>
		/// <seealso cref= PApplet#noiseDetail(int, float) </seealso>
		/// <seealso cref= PApplet#random(float,float) </seealso>
		...
		/// <seealso cref= PApplet#noise(float, float, float) </seealso>
		public virtual void noiseDetail(int lod)
		{
			if (lod > 0)
			{
				perlin_octaves = lod;
		...
		/// <param name="falloff"> falloff factor for each octave </param>
		public virtual void noiseDetail(int lod, float falloff)
		{
			if (lod > 0)
			{
				perlin_octaves = lod;
		...
		/// <seealso cref= PApplet#random(float,float) </seealso>
		/// <seealso cref= PApplet#randomSeed(long) </seealso>
		public virtual void noiseSeed(long seed)
		{
			if (perlinRandom == null)
			{
				perlinRandom = new Random();
		...
		protected internal string[] loadImageFormats;
		...
		/// <param name="filename"> name of file to load, can be .gif, .jpg, .tga, or a handful of other image types depending on your platform </param>
		/// <seealso cref= PImage </seealso>
		...
		public virtual PImage loadImage(string filename)
		{
		...
		/// <param name="extension"> type of image to load, for example "png", "gif", "jpg" </param>
		public virtual PImage loadImage(string filename, string extension) //, Object params) {
		{
			if (extension == null)
			{
			...
			{
				sbyte[] bytes = loadBytes(filename);
			...
			try
			{
				if (extension.Equals("jpg") || extension.Equals("jpeg") || extension.Equals("gif") || extension.Equals("png") || extension.Equals("unknown"))
				{
					...
					{
						Image awtImage = Toolkit.DefaultToolkit.createImage(bytes);
						PImage image = loadImageMT(awtImage);
						if (image.width == -1)
						{
							Console.Error.WriteLine("The file " + filename + " contains bad image data, or may not be an image.");
						}
			...
			{
				loadImageFormats = ImageIO.ReaderFormatNames;
			}
			if (loadImageFormats != null)
			{
			...
			Console.Error.WriteLine("Could not find a method to load " + filename);
			return null;
		}


		public virtual PImage requestImage(string filename)
		{
		...
		///
		/// @webref image:loading_displaying </summary>
		/// <param name="filename"> name of the file to load, can be .gif, .jpg, .tga, or a handful of other image types depending on your platform </param>
		/// <param name="extension"> the type of image to load, for example "png", "gif", "jpg" </param>
		/// <seealso cref= PImage </seealso>
		/// <seealso cref= PApplet#loadImage(String, String) </seealso>
		public virtual PImage requestImage(string filename, string extension)
		{
			PImage vessel = createImage(0, 0, PConstants_Fields.ARGB);
			AsyncImageLoader ail = new AsyncImageLoader(this, filename, extension, vessel);
			ail.Start();
			return vessel;
		}
		...
		internal volatile int requestImageCount;

		// TODO: implement async image load
//		internal class AsyncImageLoader : System.Threading.Thread
//		{
//			private readonly PApplet outerInstance;
//
//			internal string filename;
//			internal string extension;
//			internal PImage vessel;
//
//			public AsyncImageLoader(PApplet outerInstance, string filename, string extension, PImage vessel)
//			{
//				this.outerInstance = outerInstance;
...
//			public override void run()
//			{
//				while (outerInstance.requestImageCount == outerInstance.requestImageMax)
//				{
//					try
//					{
//						Thread.Sleep(10);
...
//					}
//				}
//				outerInstance.requestImageCount++;
//
//				PImage actual = outerInstance.loadImage(filename, extension);
//
...
//
		...
		///
		/// ( end auto-generated )
		/// @webref data:array_functions </summary>
		/// <param name="list"> array to sort </param>
		/// <seealso cref= PApplet#reverse(boolean[]) </seealso>
		public static sbyte[] sort(sbyte[] list)
		{
			return sort(list, list.Length);
		}

		/// <param name="count"> number of elements to sort, starting from 0 </param>
		public static sbyte[] sort(sbyte[] list, int count)
		{
			sbyte[] outgoing = new sbyte[list.Length];
			Array.Copy(list, 0, outgoing, 0, list.Length);
			Array.Sort(outgoing, 0, count);
			return outgoing;
		}
		...
		/// <param name="srcPosition"> starting position in the source array </param>
		/// <param name="dst"> the destination array of the same data type as the source array </param>
		/// <param name="dstPosition"> starting position in the destination array </param>
		/// <param name="length"> number of array elements to be copied </param>
		/// <seealso cref= PApplet#concat(boolean[], boolean[]) </seealso>
		...
		{
			Array.Copy(src, 0, dst, 0, Array.getLength(src));
		}
		...
		///
		/// @webref data:array_functions </summary>
		/// <param name="list"> the array to expand </param>
		/// <seealso cref= PApplet#shorten(boolean[]) </seealso>
		public static bool[] expand(bool[] list)
		{
			return expand(list, list.Length << 1);
		}

		/// <param name="newSize"> new size for the array </param>
		public static bool[] expand(bool[] list, int newSize)
		{
			bool[] temp = new bool[newSize];
			Array.Copy(list, 0, temp, 0, Math.Min(newSize, list.Length));
			return temp;
		}

		public static sbyte[] expand(sbyte[] list)
		{
			return expand(list, list.Length << 1);
		}

		public static sbyte[] expand(sbyte[] list, int newSize)
		{
			sbyte[] temp = new sbyte[newSize];
			Array.Copy(list, 0, temp, 0, Math.Min(newSize, list.Length));
			return temp;
		}
		...
		{
			Type type = list.GetType().GetElementType();
			object temp = Array.newInstance(type, newSize);
			Array.Copy(list, 0, temp, 0, Math.Min(Array.getLength(list), newSize));
			return temp;
		}
		...
		/// <param name="value"> new data for the array </param>
		/// <seealso cref= PApplet#shorten(boolean[]) </seealso>
		/// <seealso cref= PApplet#expand(boolean[]) </seealso>
		public static sbyte[] append(sbyte[] array, sbyte value)
		{
		...
		/// <seealso cref= PApplet#append(byte[], byte) </seealso>
		/// <seealso cref= PApplet#expand(boolean[]) </seealso>
		public static bool[] shorten(bool[] list)
		{
			return subset(list, 0, list.Length - 1);
		}

		public static sbyte[] shorten(sbyte[] list)
		{
			return subset(list, 0, list.Length - 1);
		}
		...
		/// <param name="index"> position in the array from which to insert data </param>
		/// <seealso cref= PApplet#concat(boolean[], boolean[]) </seealso>
		/// <seealso cref= PApplet#subset(boolean[], int, int) </seealso>
		public static bool[] splice(bool[] list, bool value, int index)
		{
			bool[] outgoing = new bool[list.Length + 1];
			Array.Copy(list, 0, outgoing, 0, index);
			outgoing[index] = value;
			Array.Copy(list, index, outgoing, index + 1, list.Length - index);
			return outgoing;
		}

		public static bool[] splice(bool[] list, bool[] value, int index)
		{
			bool[] outgoing = new bool[list.Length + value.Length];
			Array.Copy(list, 0, outgoing, 0, index);
			Array.Copy(value, 0, outgoing, index, value.Length);
			Array.Copy(list, index, outgoing, index + value.Length, list.Length - index);
			return outgoing;
		}

		public static sbyte[] splice(sbyte[] list, sbyte value, int index)
		{
			sbyte[] outgoing = new sbyte[list.Length + 1];
			Array.Copy(list, 0, outgoing, 0, index);
			outgoing[index] = value;
			Array.Copy(list, index, outgoing, index + 1, list.Length - index);
			return outgoing;
		...
		public static sbyte[] splice(sbyte[] list, sbyte[] value, int index)
		{
			sbyte[] outgoing = new sbyte[list.Length + value.Length];
			Array.Copy(list, 0, outgoing, 0, index);
			Array.Copy(value, 0, outgoing, index, value.Length);
			Array.Copy(list, index, outgoing, index + value.Length, list.Length - index);
			return outgoing;
		...
		public static char[] splice(char[] list, char value, int index)
		{
		...
		public static int[] splice(int[] list, int value, int index)
		{
		...
		public static float[] splice(float[] list, float value, int index)
		{
		...
		public static string[] splice(string[] list, string value, int index)
		{
		...
		public static object splice(object list, object value, int index)
		{
			...
			//JAVA TO C# CONVERTER WARNING: The .NET Type.FullName property will not always yield results identical to the Java Class.getName method:
			if (value.GetType().FullName[0] == '[')
			{
				int vlength = Array.getLength(value);
				outgoing = new object[length + vlength];
				Array.Copy(list, 0, outgoing, 0, index);
				Array.Copy(value, 0, outgoing, index, vlength);
				Array.Copy(list, index, outgoing, index + vlength, length - index);
			...
			else
			{
				...
				Array.Copy(list, index, outgoing, index + 1, length - index);
			}
			return outgoing;
		}

		public static bool[] subset(bool[] list, int start)
		{
		...
		/// <seealso cref= PApplet#splice(boolean[], boolean, int) </seealso>
		public static bool[] subset(bool[] list, int start, int count)
		{
			bool[] output = new bool[count];
			Array.Copy(list, start, output, 0, count);
			return output;
		}

		public static sbyte[] subset(sbyte[] list, int start)
		{
			return subset(list, start, list.Length - start);
		}

		public static sbyte[] subset(sbyte[] list, int start, int count)
		{
		...
		public static object subset(object list, int start, int count)
		{
			...
			Array.Copy(list, start, outgoing, 0, count);
			return outgoing;
		}
		...
		/// <seealso cref= PApplet#splice(boolean[], boolean, int) </seealso>
		/// <seealso cref= PApplet#arrayCopy(Object, int, Object, int, int) </seealso>
		public static bool[] concat(bool[] a, bool[] b)
		{
			bool[] c = new bool[a.Length + b.Length];
		...
		{
			sbyte[] c = new sbyte[a.Length + b.Length];
			Array.Copy(a, 0, c, 0, a.Length);
			Array.Copy(b, 0, c, a.Length, b.Length);
			return c;
		}
		...
		public static object concat(object a, object b)
		{
			...
			Array.Copy(a, 0, outgoing, 0, alength);
			Array.Copy(b, 0, outgoing, alength, blength);
			return outgoing;
		}
		...
		/// ( end auto-generated )
		/// @webref data:array_functions </summary>
		/// <param name="list"> booleans[], bytes[], chars[], ints[], floats[], or Strings[] </param>
		/// <seealso cref= PApplet#sort(String[], int) </seealso>
		public static bool[] reverse(bool[] list)
		{
			bool[] outgoing = new bool[list.Length];
		...
		public static sbyte[] reverse(sbyte[] list)
		{
			sbyte[] outgoing = new sbyte[list.Length];
			int length1 = list.Length - 1;
			for (int i = 0; i < list.Length; i++)
		...
		/// <param name="separator"> char or String to be placed between each item </param>
		/// <seealso cref= PApplet#split(String, String) </seealso>
		/// <seealso cref= PApplet#trim(String) </seealso>
		/// <seealso cref= PApplet#nf(float, int, int) </seealso>
		/// <seealso cref= PApplet#nfs(float, int, int) </seealso>
		public static string join(string[] list, char separator)
		{
			return join(list, Convert.ToString(separator));
		}
		...
		{
			return splitTokens(value, PConstants_Fields.WHITESPACE);
		}
		...
		/// <param name="delim"> list of individual characters that will be used as separators </param>
		/// <seealso cref= PApplet#split(String, String) </seealso>
		...
		/// @usage web_application </summary>
		/// <param name="value"> the String to be split </param>
		/// <param name="delim"> the character or String used to separate the data </param>
		...
		public static string[] Split(string value, string delim)
		{
			...
			{
				items.Add(value.Substring(offset, index - offset));
				offset = index + delim.Length;
			}
			items.Add(value.Substring(offset));
			string[] outgoing = new string[items.Count];
...
//
		...
		*/
		...
		/// <summary>
		/// <para>Convert an integer to a boolean. Because of how Java handles upgrading
		/// numbers, this will also cover byte and char (as they will upgrade to
		/// an int without any sort of explicit cast).</para>
		/// <para>The preprocessor will convert boolean(what) to parseBoolean(what).</para> </summary>
		/// <returns> false if 0, true if any other number </returns>
		public static bool parseBoolean(int what)
		{
			return (what != 0);
		}
		...
		*/
		...
		/// <summary>
		/// Convert the string "true" or "false" to a boolean. </summary>
		/// <returns> true if 'what' is "true" or "TRUE", false otherwise </returns>
		public static bool parseBoolean(string what)
		{
			return (bool)(new bool? (what));
		}
		...
		*/
		...
		/// to zero will return false, and any other value will return true. </summary>
		/// <returns> array of boolean elements </returns>
		/*
		...
		*/
		...
		public static bool[] parseBoolean(int[] what)
		{
			bool[] outgoing = new bool[what.Length];
			for (int i = 0; i < what.Length; i++)
			{
				outgoing[i] = (what[i] != 0);
			}
		...
		*/
		...
		public static bool[] parseBoolean(string[] what)
		{
			...
			for (int i = 0; i < what.Length; i++)
			{
				outgoing[i] = (bool)(new bool? (what[i]));
			}
			return outgoing;
		}
		...
		public static sbyte parseByte(bool what)
		{
			return what ? (sbyte)1 : 0;
		}

		public static sbyte parseByte(char what)
		{
			return (sbyte) what;
		}

		public static sbyte parseByte(int what)
		{
			return (sbyte) what;
		}

		public static sbyte parseByte(float what)
		{
			return (sbyte) what;
		}
		...
		*/
		...
		public static sbyte[] parseByte(bool[] what)
		{
			sbyte[] outgoing = new sbyte[what.Length];
			for (int i = 0; i < what.Length; i++)
			{
				outgoing[i] = what[i] ? (sbyte)1 : 0;
			}
			return outgoing;
		}
		...
		*/
		...
		public static char parseChar(sbyte what)
		{
			return (char)(what & 0xff);
		}

		public static char parseChar(int what)
		{
			return (char) what;
		}
		...
		*/
		...
		public static int parseInt(bool what)
		{
			return what ? 1 : 0;
		}
		...
		/// </summary>
		public static int parseInt(sbyte what)
		{
			return what & 0xff;
		}
		...
		/// </summary>
		public static int parseInt(char what)
		{
			return what;
		}
		...
		/// </summary>
		public static int parseInt(string what, int otherwise)
		{
			try
			{
				...
				{
					return Convert.ToInt32(what.Substring(0, offset));
				}
			}
		...
		public static int[] parseInt(sbyte[] what) // note this unsigns
		{
			int[] list = new int[what.Length];
			for (int i = 0; i < what.Length; i++)
			{
		...
		public static int[] parseInt(string[] what, int missing)
		{
			...
			for (int i = 0; i < what.Length; i++)
			{
				try
				{
					output[i] = Convert.ToInt32(what[i]);
				}
				catch (NumberFormatException)
				{
					output[i] = missing;
		...
		*/
		...
		/// </summary>
		public static float parseFloat(int what) // also handles byte
		{
			return what;
		}

		public static float parseFloat(string what)
		{
			return parseFloat(what, float.NaN);
		}

		public static float parseFloat(string what, float otherwise)
		{
			try
			{
				return (float)(Convert.ToSingle(what));
			}
			catch (NumberFormatException)
			{
			}
		...
		*/
		...
		public static float[] parseByte(sbyte[] what)
		{
			float[] floaties = new float[what.Length];
			for (int i = 0; i < what.Length; i++)
			{
		...
		public static float[] parseFloat(int[] what)
		{
			float[] floaties = new float[what.Length];
			for (int i = 0; i < what.Length; i++)
			{
		...
		public static float[] parseFloat(string[] what, float missing)
		{
			...
			for (int i = 0; i < what.Length; i++)
			{
				try
				{
					output[i] = (float)(Convert.ToSingle(what[i]));
				}
				catch (NumberFormatException)
				{
					output[i] = missing;
		...
		public static string str(bool x)
		{
			return Convert.ToString(x);
		}

		public static string str(sbyte x)
		{
			return Convert.ToString(x);
		}
...
//		private static bool int_nf_commas;
		...
		/// ( end auto-generated )
		/// @webref data:string_functions </summary>
		/// <param name="num"> the number(s) to format </param>
		/// <param name="digits"> number of digits to pad with zero </param>
		/// <seealso cref= PApplet#nfs(float, int, int) </seealso>
		/// <seealso cref= PApplet#nfp(float, int, int) </seealso>
		/// <seealso cref= PApplet#nfc(float, int) </seealso>
		/// <seealso cref= PApplet#int(float) </seealso>
		public static string nf(int num, int digits)
		{
			return String.Format("{0:D" + digits + "}", number);
...
//			}
//
//			int_nf = NumberFormat.Instance;
//			int_nf.GroupingUsed = false; // no commas
//			int_nf_commas = false;
...
//
//			int_nf = NumberFormat.Instance;
//			int_nf.GroupingUsed = true;
//			int_nf_commas = true;
//			int_nf.MinimumIntegerDigits = 0;
//			int_nf_digits = 0;
//			return int_nf.format(num);
		}
		...
		/// @webref data:string_functions </summary>
		/// <param name="num"> the number(s) to format </param>
		/// <param name="digits"> number of digits to pad with zeroes </param>
		/// <seealso cref= PApplet#nf(float, int, int) </seealso>
		/// <seealso cref= PApplet#nfp(float, int, int) </seealso>
		/// <seealso cref= PApplet#nfc(float, int) </seealso>
...
//		private static bool float_nf_commas;
		...
		/// <param name="num">[] the number(s) to format </param>
		/// <param name="left"> number of digits to the left of the decimal point </param>
		/// <param name="right"> number of digits to the right of the decimal point </param>
		public static string nf(float num, int left, int right)
		{
			//TODO: temporary impl
			return string.Format ("{0,F"+ right +"}", num);
//			if ((float_nf != null) && (float_nf_left == left) && (float_nf_right == right) && !float_nf_commas)
//			{
//				return float_nf.format(num);
//			}

//			float_nf = NumberFormat.Instance;
//			float_nf.GroupingUsed = false;
//			float_nf_commas = false;
//
//			if (left != 0)
//			{
//				float_nf.MinimumIntegerDigits = left;
//			}
//			if (right != 0)
//			{
//				float_nf.MinimumFractionDigits = right;
//				float_nf.MaximumFractionDigits = right;
//			}
//			float_nf_left = left;
...
//			return float_nf.format(num);
		...
		/// <param name="num">[] the number(s) to format </param>
		/// <param name="left"> the number of digits to the left of the decimal point </param>
		/// <param name="right"> the number of digits to the right of the decimal point </param>
		...
		///
		/// ( end auto-generated )
		/// @webref data:conversion </summary>
		/// <param name="value"> the value to convert </param>
		/// <seealso cref= PApplet#unhex(String) </seealso>
		/// <seealso cref= PApplet#binary(byte) </seealso>
		/// <seealso cref= PApplet#unbinary(String) </seealso>
		...
		/// <param name="digits"> the number of digits (maximum 8) </param>
		public static string hex(int value, int digits)
		{
			string stuff = value.ToString("x").ToUpper();
			if (digits > 8)
			{
				digits = 8;
			}
		...
		/// <seealso cref= PApplet#hex(int, int) </seealso>
		/// <seealso cref= PApplet#binary(byte) </seealso>
		/// <seealso cref= PApplet#unbinary(String) </seealso>
		...
		*/
		...
		{
			string stuff = int.toBinaryString(value);
			if (digits > 32)
			{
				digits = 32;
			}
		...
		///
		/// ( end auto-generated )
		/// @webref color:creating_reading </summary>
		/// <param name="gray"> number specifying value between white and black </param>
		/// <seealso cref= PApplet#colorMode(int) </seealso>
		...
		/// <summary>
		/// @nowebref </summary>
		/// <param name="fgray"> number specifying value between white and black </param>
		...
		/// <summary>
		/// As of 0116 this also takes color(#FF8800, alpha) </summary>
		/// <param name="alpha"> relative to current color range </param>
		...
		/// <param name="v1"> red or hue values relative to the current color range </param>
		/// <param name="v2"> green or saturation values relative to the current color range </param>
		/// <param name="v3"> blue or brightness values relative to the current color range </param>
		public int color(int v1, int v2, int v3)
		{
			if (g == null)
			{
				...
				return unchecked((int)0xff000000) | (v1 << 16) | (v2 << 8) | v3;
			}
		...
		public int color(float v1, float v2, float v3)
		{
			if (g == null)
			{
				...
				return unchecked((int)0xff000000) | ((int)v1 << 16) | ((int)v2 << 8) | (int)v3;
			}
			return g.color(v1, v2, v3);
		}
		...
		/// </summary>
		public virtual void setupExternalMessages()
		{

			//TODO: implement if necessary
//			frame.addComponentListener(new ComponentAdapterxternalMessageHelper(this));
//
//			frame.addWindowListener(new WindowAdapterAnonymousInnerClassHelper(this));
		}

//		private class ComponentAdapterxternalMessageHelper : ComponentAdapter
//		{
//			private readonly PApplet outerInstance;
//
//			public ComponentAdapterxternalMessageHelper(PApplet outerInstance)
//			{
//				this.outerInstance = outerInstance;
//			}
//
...
//			{
//				Point @where = ((Frame) e.Source).Location;
//				Console.Error.WriteLine(PApplet.EXTERNAL_MOVE + " " + @where.x + " " + @where.y);
...
//			private readonly PApplet outerInstance;
//
//			public WindowAdapterAnonymousInnerClassHelper(PApplet outerInstance)
//			{
//				this.outerInstance = outerInstance;
//			}
//
...
//				//          System.exit(0);
//				outerInstance.exit(); // don't quit, need to just shut everything down (0133)
//			}
//		}
		...
		/// </summary>
		public virtual void setupFrameResizeListener()
		{
			// TODO: implement if necessary
//			frame.addComponentListener(new ComponentAdapterAnonymousInnerClassHelper2(this));
		}

//		private class ComponentAdapterAnonymousInnerClassHelper2 : ComponentAdapter
//		{
//			private readonly PApplet outerInstance;
//
//			public ComponentAdapterAnonymousInnerClassHelper2(PApplet outerInstance)
//			{
//				this.outerInstance = outerInstance;
//			}
//
...
//				if (outerInstance.frame.Resizable)
//				{
//					// might be multiple resize calls before visible (i.e. first
//					// when pack() is called, then when it's resized for use).
//					// ignore them because it's not the user resizing things.
//					Frame farm = (Frame) e.Component;
//					if (farm.Visible)
//					{
//						Insets insets = farm.Insets;
//						Dimension windowSize = farm.Size;
...
//						Rectangle newBounds = new Rectangle(0, 0, windowSize.width - insets.left - insets.right, windowSize.height - insets.top - insets.bottom); //insets.left, insets.top,
...
//		}
		...
		// TODO: implement if necessary
//		internal static List<Image> iconImages;
//
//		protected internal virtual Frame IconImage
//		{
//			set
//			{
...
//							//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//							//ORIGINAL LINE: final int[] sizes = { 16, 32, 48, 64 };
//							int[] sizes = new int[] {16, 32, 48, 64};
//
//							foreach (int sz in sizes)
//							{
//								URL url = this.GetType().getResource("/icon/icon-" + sz + ".png");
//								Image image = Toolkit.DefaultToolkit.getImage(url);
//								iconImages.Add(image);
//								//iconImages.add(Toolkit.getLibImage("icons/pde-" + sz + ".png", value));
//							}
//						}
//						value.IconImages = iconImages;
//					}
//					catch (Exception)
//					{
...
//
		...
		/// rather than having to wrap it into a String array. </summary>
		/// <param name="mainClass"> name of the class to load (with package if any) </param>
		//JAVA TO C# CONVERTER WARNING: 'final' parameters are not allowed in .NET:
		//ORIGINAL LINE: public static void main(final String mainClass)
		...
		/// the 'args' array when not null. </summary>
		/// <param name="mainClass"> name of the class to load (with package if any) </param>
		/// <param name="args"> command line arguments to pass to the sketch </param>
		//JAVA TO C# CONVERTER WARNING: 'final' parameters are not allowed in .NET:
		//ORIGINAL LINE: public static void main(final String mainClass, final String[] passedArgs)
		public static void Main(string mainClass, string[] passedArgs)
		{
		...
		//JAVA TO C# CONVERTER WARNING: 'final' parameters are not allowed in .NET:
		//ORIGINAL LINE: public static void runSketch(final String args[] , final PApplet constructedApplet)
		public static void runSketch(string[] args, PApplet constructedApplet)
		{
			...
			if (platform == PConstants_Fields.MACOSX)
			{
				...
				System.setProperty("apple.awt.graphics.UseQuartz", Convert.ToString(useQuartz));
			}
			...
			if (args.Length < 1)
			{
				Console.Error.WriteLine("Usage: PApplet <appletname>");
				Console.Error.WriteLine("For additional options, " + "see the Javadoc for PApplet");
				Environment.Exit(1);
			}
			...
			while (argIndex < args.Length)
			{
				...
				if (equals != -1)
				{
					...
					{
						int deviceIndex = Convert.ToInt32(value);

						GraphicsEnvironment environment = GraphicsEnvironment.LocalGraphicsEnvironment;
						GraphicsDevice[] devices = environment.ScreenDevices;
						...
						{
							Console.Error.WriteLine("Display " + value + " does not exist, " + "using the default display instead.");
							for (int i = 0; i < devices.Length; i++)
							{
								Console.Error.WriteLine(i + " is " + devices[i]);
							}
						}
					...
					else if (param.Equals(ARGS_BGCOLOR))
					{
						...
						backgroundColor = new Color(Convert.ToInt32(value, 16));
					...
					else if (param.Equals(ARGS_STOP_COLOR))
					{
						...
						stopColor = new Color(Convert.ToInt32(value, 16));
			...
			{
				GraphicsEnvironment environment = GraphicsEnvironment.LocalGraphicsEnvironment;
				displayDevice = environment.DefaultScreenDevice;
			}
			...
			Frame frame = new JFrame(displayDevice.DefaultConfiguration);
			// Default Processing gray, which will be replaced below if another
			// color is specified on the command line (i.e. in the prefs).
			//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
			//ORIGINAL LINE: final Color defaultGray = new Color(0xCC, 0xCC, 0xCC);
			Color defaultGray = new Color(0xCC, 0xCC, 0xCC);
			((JFrame) frame).ContentPane.Background = defaultGray;
			// Cannot call setResizable(false) until later due to OS X (issue #467)

			//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
			//ORIGINAL LINE: final PApplet applet;
			PApplet applet;
			...
			else
			{
				try
				{
					Type c = Thread.CurrentThread.ContextClassLoader.loadClass(name);
			...
			applet.frame = frame;
			applet.sketchPath_Renamed = folder;
			...
			if (present)
			{
				...
				{
					((JFrame) frame).ContentPane.Background = backgroundColor;
				}
				//      if (exclusive) {
			...
			if (present)
			{
				...
				if (!hideStop)
				{
					...
					label.Foreground = stopColor;
					label.addMouseListener(new MouseAdapterAnonymousInnerClassHelper());
					frame.add(label);

					Dimension labelSize = label.PreferredSize;
			...
			else
			{
				...
				{
					frame.addWindowListener(new WindowAdapterAnonymousInnerClassHelper2());
				}
				...
				if (applet.displayable())
				{
					...
					if (platform == PConstants_Fields.LINUX)
					{
						Insets irlInsets = frame.Insets;
...
//		{
//			public MouseAdapterAnonymousInnerClassHelper()
//			{
//			}
//
...
//		}
//
//		private class WindowAdapterAnonymousInnerClassHelper2 : WindowAdapter
//		{
//			public WindowAdapterAnonymousInnerClassHelper2()
//			{
//			}
//
//			public override void windowClosing(WindowEvent e)
...
//		}
		...
		/// </summary>
		//JAVA TO C# CONVERTER WARNING: 'final' parameters are not allowed in .NET:
		//ORIGINAL LINE: protected void runSketch(final String[] args)
		protected internal virtual void runSketch(string[] args)
		{
			//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
			//ORIGINAL LINE: final String[] argsWithSketchName = new String[args.length + 1];
			string[] argsWithSketchName = new string[args.Length + 1];
			Array.Copy(args, 0, argsWithSketchName, 0, args.Length);
			//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
			//ORIGINAL LINE: final String className = this.getClass().getSimpleName();
			string className = this.GetType().Name;
			//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
			//ORIGINAL LINE: final String cleanedClass = className.replaceAll("__[^_]+__\\$", "").replaceAll("\\$\\d+", "");
			string cleanedClass = className.replaceAll("__[^_]+__\\$", "").replaceAll("\\$\\d+", "");
			argsWithSketchName[args.Length] = cleanedClass;
			runSketch(argsWithSketchName, this);
		}


		protected internal virtual void runSketch()
		{
		...
		///
		/// @webref output:files </summary>
		/// <param name="renderer"> for example, PDF </param>
		/// <param name="filename"> filename for output </param>
		/// <seealso cref= PApplet#endRecord() </seealso>
		public virtual PGraphics beginRecord(string renderer, string filename)
		{
		...
		/// </summary>
		public virtual void beginRecord(PGraphics recorder)
		{
		...
		/// ( end auto-generated )
		/// @webref output:files </summary>
		/// <seealso cref= PApplet#beginRecord(String, String) </seealso>
		...
		///
		/// @webref output:files </summary>
		/// <param name="renderer"> for example, PDF or DXF </param>
		/// <param name="filename"> filename for output </param>
		/// <seealso cref= PApplet#endRaw() </seealso>
		/// <seealso cref= PApplet#hint(int) </seealso>
		public virtual PGraphics beginRaw(string renderer, string filename)
		{
			filename = insertFrame(filename);
			PGraphics rec = createGraphics(width, height, renderer, filename);
			g.beginRaw(rec);
		...
		/// </summary>
		/// <param name="rawGraphics"> ??? </param>
		public virtual void beginRaw(PGraphics rawGraphics)
		{
		...
		///
		/// @webref output:files </summary>
		/// <seealso cref= PApplet#beginRaw(String, String) </seealso>
		public virtual void endRaw()
		{
			g.endRaw();
		}
		...
		*/
		...
		/// <seealso cref= PApplet#updatePixels() </seealso>
		public virtual void loadPixels()
		{
			g.loadPixels();
			pixels = g.pixels;
		}
		...
		/// <seealso cref= PApplet#loadPixels() </seealso>
		/// <seealso cref= PApplet#pixels </seealso>
		public virtual void updatePixels()
		{
			g.updatePixels();
		}
		...
		/// <param name="x2"> width of the region </param>
		/// <param name="y2"> height of the region </param>
		public virtual void updatePixels(int x1, int y1, int x2, int y2)
		{
